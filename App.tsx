...
Make sure every question has all options and the correct answer letter.`;
      ...
      for (const model of sorted) {
        ...
        setQuestions(parsed);
        // ── Auto-save questions so test can be re-attempted without re-uploading PDF
        const saved = saveTestToStorage(file.name, parsed);
        setSavedTests(getSavedTests());
        setActiveTestName(file.name);
        addLog(`✓ ${parsed.length} questions found! Test saved to library.`, 'ok');
        setTimeout(() => { setScreen('test'); setLoading(false); }, 700);
        return;
      }

      throw new Error(
        `All ${sorted.length} models hit their free-tier quota.\n\n` +
        `Fix options:\n` +
        `1. Wait until midnight Pacific Time for quota reset\n` +
        `2. Add billing at console.cloud.google.com\n` +
        `3. Create a new key at aistudio.google.com/app/apikey`
      );
    } catch (err: any) {
...
Correct: ${q.answer}. ${q.options[q.answer]}`;
  ...
  const restartApp = () => {
    ...
    setSavedTests(getSavedTests()); // refresh library in case any changes
                ...
                <div key={t.id} className="saved-item" style={{ animationDelay: `${i * 0.05}s` }}>
                  ...
                  <div className="saved-info">
                    ...
                    <div className="saved-meta">
                      ...
                      <span>{new Date(t.savedAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })}</span>
                    </div>
                  </div>
                  <div className="saved-actions">
                    <button className="btn-load" onClick={() => loadSavedTest(t)}>
