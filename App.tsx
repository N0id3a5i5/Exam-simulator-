...
export default function App() {
  ...
  const startExtraction = async () => {
    ...
    try {
      ...
      // Priority: 2.0-flash > 1.5-pro > 1.5-flash > anything else
      const bestModel =
      ...
      if (!bestModel) throw new Error('No compatible Gemini model found for your API key');
...
Make sure every question has all options and the correct answer letter.`;
      ...
      const res = await fetch(
        `https://generativelanguage.googleapis.com/v1beta/${bestModel}:generateContent?key=${apiKey}`,
        {
          method: 'POST',
    ...
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', padding: 16 }}>
      {loading ? (
        ...
        <div className="upload-card">
          ...
          <p style={{ fontSize: 13, color: 'var(--muted)', textAlign: 'center', marginBottom: 16 }}>Upload a question paper PDF to begin your timed test</p>
          ...
          <div style={{
            ...
            <span style={{ color: 'var(--green)', fontWeight: 700 }}>Auto-detects best Gemini model</span>
          </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="review-answers">
                    {sk ? (
                      <span className="ans-tag skipped">Skipped</span>
                    ) : (
                      <span className={`ans-tag ${ok ? 'your-correct' : 'your-wrong'}`}>
                        You: {selections[q.id]}
                      </span>
                    )}
                    {!ok && <span className="ans-tag correct-ans">Correct: {q.answer}</span>}
                  </div>
                </div>
              );
            })}
