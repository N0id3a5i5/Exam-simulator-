...
export default function App() {
  ...
  // ── GROQ extraction — v9: all models fired in parallel, first success wins
  // Previous: sequential waterfall — rate-limit on model A blocked B and C from starting.
  // Now: all 3 models start simultaneously via Promise.any. Rate limit on one doesn’t
  // delay the others. Chunks within each model are still sequential (each needs chunk index).
  const tryGroq = async (pdfText: string, gKey: string): Promise<Question[] | null> => {
    ...
    for (let i = 0; i < pdfText.length; i += CHUNK_SIZE) textChunks.push(pdfText.slice(i, i + CHUNK_SIZE));
    if (textChunks.length === 0) textChunks.push(pdfText);

    const makePrompt = (text: string, ci: number, total: number) =>
      `Extract ALL multiple choice questions from this PDF text chunk (${ci + 1} of ${total}).\n` +
      `Return ONLY a valid JSON array. No markdown, no explanation, no preamble.\n` +
      `Format: [{"id":1,"question":"...","options":{"A":"...","B":"...","C":"...","D":"..."},"answer":"B"}]\n` +
      `If no MCQ questions exist in this chunk, return an empty array: []\n\nPDF TEXT:\n${text}`;

    // Run ONE model across all its chunks; throw on rate-limit/error so Promise.any skips it
    const runModel = async (model: string, maxTokens: number): Promise<Question[]> => {
      const shortName = model.split('/').pop()!;
      addLog(`[Groq] Starting ${shortName}...`, 'info');
      const allQuestions: Question[] = [];

      for (let ci = 0; ci < textChunks.length; ci++) {
        if (textChunks.length > 1) addLog(`[Groq:${shortName}] chunk ${ci + 1}/${textChunks.length}`, 'info');
        ...
        const data = await capFetch('https://api.groq.com/openai/v1/chat/completions', {
          ...
          body: JSON.stringify({
            model,
            messages: [{ role: 'user', content: makePrompt(textChunks[ci], ci, textChunks.length) }],
            temperature: 0.1,
        ...
        if (data.error?.code === 429 || data.error?.message?.toLowerCase().includes('rate')) {
          addLog(`[Groq:${shortName}] rate-limited — model skipped`, 'warn');
          throw new Error(`${shortName}: rate-limited`);
        }
        if (data.error) {
          addLog(`[Groq:${shortName}] error: ${data.error.message}`, 'warn');
          throw new Error(`${shortName}: ${data.error.message}`);
        }
        if (data.choices?.[0]?.finish_reason === 'length')
          addLog(`[Groq:${shortName}] chunk ${ci + 1} truncated — partial recovery`, 'warn');

        const rawText: string = data.choices?.[0]?.message?.content || '';
        let chunkQs: Question[] = [];
        try {
          const parsed = JSON.parse(rawText);
          if (Array.isArray(parsed)) chunkQs = validateQuestions(parsed);
          else {
            const arr = (Object.values(parsed) as any[]).find(v => Array.isArray(v));
            if (arr) chunkQs = validateQuestions(arr);
          }
        } catch { chunkQs = validateQuestions(parseQuestionsFromLLM(rawText)); }

        allQuestions.push(...chunkQs);
      }

      if (allQuestions.length === 0) throw new Error(`${shortName}: no valid questions extracted`);
      allQuestions.forEach((q, i) => { q.id = i + 1; });
      addLog(`[Groq:${shortName}] ✓ ${allQuestions.length} questions`, 'ok');
      return allQuestions;
    };

    // Fire all models simultaneously — first success wins, rate-limited ones ignored
    addLog(`[Groq] Racing ${GROQ_MODELS.length} models simultaneously...`, 'info');
    try {
      return await Promise.any(GROQ_MODELS.map(({ model, maxTokens }) => runModel(model, maxTokens)));
    } catch {
      addLog('[Groq] All models rate-limited or failed', 'warn');
      return null;
    }
  };
