...
export default function App() {
  ...
  // ── Extraction: tries every available model in priority order ─────────────
  const startExtraction = async () => {
    ...
    setLoading(true);
    addLog('Checking key permissions...', 'info');
    try {
      ...
      addLog(`Found ${sorted.length} model(s) — will auto-fallback if quota exceeded`, 'info');
...
Make sure every question has all options and the correct answer letter.`;
      ...
      for (const model of sorted) {
        ...
        setDiscoveredModel(shortName);
        addLog(`✓ Using ${shortName}`, 'ok');
        addLog('Extracting MCQs (10–30s)...', 'info');

        const rawText = data.candidates[0].content.parts[0].text as string;
        addLog('Parsing extracted questions...', 'info');
        const parsed = parseQuestionsFromLLM(rawText);
        if (parsed.length === 0) throw new Error('No questions found in PDF — try a different PDF');
        // Validate each question has required fields
        const valid = parsed.filter(q => q.question && q.options && q.answer && Object.keys(q.options).length >= 2);
        if (valid.length === 0) throw new Error('Questions found but none had valid format');
        // Re-index IDs to ensure they are sequential
        valid.forEach((q, i) => { q.id = i + 1; });

        const saved = saveTestToStorage(file.name, valid);
        setSavedTests(getSavedTests());
        setActiveTestName(file.name);
        setActiveTestId(saved.id);
        setQuestions(valid);
        setStartTime(Date.now());
        addLog(`✓ ${valid.length} questions extracted (${parsed.length - valid.length} invalid skipped). Saved!`, 'ok');
      ...
      if (gKey) {
        addLog('All Gemini quota exceeded — switching to Groq...', 'warn');
        addLog('Extracting PDF text for Groq...', 'info');
        try {
          const pdfText = await extractPdfText(file);
          if (!pdfText || pdfText.length < 100) throw new Error('Could not extract readable text from PDF');
          ...
          for (const gModel of groqModels) {
            ...
            if (gData.error?.message?.toLowerCase().includes('rate') ||
                gData.error?.code === 429) {
              addLog(`Groq ${gModel} rate limited — trying next`, 'warn'); continue;
            }
            if (gData.error) throw new Error(`Groq: ${gData.error.message}`);
            const groqRawText = gData.choices[0].message.content as string;
            const parsed = parseQuestionsFromLLM(groqRawText);
            if (parsed.length === 0) throw new Error('Groq returned no questions');
            const valid = parsed.filter(q => q.question && q.options && q.answer && Object.keys(q.options).length >= 2);
            if (valid.length === 0) throw new Error('Groq questions had invalid format');
            valid.forEach((q, i) => { q.id = i + 1; });
            setDiscoveredModel(`groq:${gModel}`);
            const saved = saveTestToStorage(file.name, valid);
            setSavedTests(getSavedTests());
            setActiveTestName(file.name);
            setActiveTestId(saved.id);
            setQuestions(valid);
            setStartTime(Date.now());
            addLog(`✓ Groq extracted ${valid.length} questions! Test saved.`, 'ok');
            setTimeout(() => { setScreen('test'); setLoading(false); }, 700);
            return;
          }
        } catch (groqErr: any) {
          addLog(`Groq error: ${groqErr.message}`, 'err');
        }
      }

      throw new Error(
        `All Gemini models hit quota${groqKey ? ' and Groq also failed' : ''}.\n` +
        `Fix: wait for quota reset, add billing, or add a Groq key (groq.com — free).`
      );
    } catch (err: any) {
