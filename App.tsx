...
// Returns per-page text array so callers can split at page boundaries.
...
const extractPdfText = async (file: File): Promise<string> =>
  (await extractPdfPages(file)).join('\n').trim();
...
export default function App() {
  ...
  // ── v6 FIX: Updated model list. Removed decommissioned mixtral-8x7b-32768
  //    and llama3-70b-8192. Added llama-4-scout and llama-3.1-8b-instant.
  //    Using response_format json_object to prevent markdown wrapping.
  //    Chunk large PDFs into 16k-char segments and merge results to avoid truncation.
  const tryGroq = async (pdfText: string, gKey: string): Promise<Question[] | null> => {
    // ── Current active Groq models (as of March 2026)
    // llama-3.3-70b-versatile: 128k context, best quality
    // meta-llama/llama-4-scout-17b-16e-instruct: 128k context, fast MoE
    // llama-3.1-8b-instant: 128k context, lightweight fallback
    const groqModels = [
      { model: 'llama-3.3-70b-versatile',                   maxTokens: 8000 },
      { model: 'meta-llama/llama-4-scout-17b-16e-instruct', maxTokens: 8000 },
      { model: 'llama-3.1-8b-instant',                      maxTokens: 8000 },
    ];

    // ── Chunk strategy: send 14k-char slices and merge
    // This avoids both context overflow AND output truncation.
    // Each slice is small enough that the JSON response fits in 8k tokens.
    const CHUNK_SIZE = 14000;
    const chunks: string[] = [];
    for (let i = 0; i < pdfText.length; i += CHUNK_SIZE) {
      chunks.push(pdfText.slice(i, i + CHUNK_SIZE));
    }
    // If the whole text is short, just use one chunk
    const textChunks = chunks.length > 0 ? chunks : [pdfText];

    const makePrompt = (text: string, chunkIdx: number, totalChunks: number) =>
      `Extract ALL multiple choice questions from this PDF text chunk (${chunkIdx + 1} of ${totalChunks}).\n` +
      `Return ONLY a valid JSON array. No markdown, no explanation, no preamble.\n` +
      `Format: [{"id":1,"question":"...","options":{"A":"...","B":"...","C":"...","D":"..."},"answer":"B"}]\n` +
      `If no MCQ questions exist in this chunk, return an empty array: []\n\n` +
      `PDF TEXT:\n${text}`;

    for (const { model, maxTokens } of groqModels) {
      addLog(`Trying Groq ${model.split('/').pop()}...`, 'info');
      ...
      for (let ci = 0; ci < textChunks.length; ci++) {
        ...
        if (textChunks.length > 1) {
          addLog(`  Processing chunk ${ci + 1}/${textChunks.length}...`, 'info');
        }
        try {
          const requestBody: any = {
            model,
            messages: [{ role: 'user', content: makePrompt(chunk, ci, textChunks.length) }],
            temperature: 0.1,
            max_tokens: maxTokens,
            // Force JSON output — prevents markdown fences and prose preamble
            response_format: { type: 'json_object' },
          };

          const res = await capFetch('https://api.groq.com/openai/v1/chat/completions', {
            method: 'POST',
          ...
          if (data.error?.code === 429 || data.error?.message?.toLowerCase().includes('rate')) {
            addLog(`Groq ${model.split('/').pop()} rate-limited — trying next model`, 'warn');
            modelFailed = true;
            break;
          }
          // Model-level error (e.g. decommissioned) — try next model
          if (data.error) {
            addLog(`Groq ${model.split('/').pop()} error: ${data.error.message}`, 'warn');
            modelFailed = true;
            break;
          }
          ...
          if (finishReason === 'length') {
            // Even with chunking this chunk overflowed — log but still try to recover
            addLog(`  Chunk ${ci + 1} response truncated — attempting partial recovery`, 'warn');
          }

          const rawText: string = data.choices?.[0]?.message?.content || '';

          // json_object mode returns {"questions":[...]} or directly [...] wrapped in an object
          // parseQuestionsFromLLM handles both cases via array-finding logic
          let chunkQuestions: Question[] = [];
          try {
            // First try: parse as JSON object and look for array inside
            const parsed = JSON.parse(rawText);
            if (Array.isArray(parsed)) {
              chunkQuestions = validateQuestions(parsed);
            } else {
              // Find the first array value in the object
              const arr = (Object.values(parsed) as any[]).find(v => Array.isArray(v));
              if (arr) chunkQuestions = validateQuestions(arr);
            }
          } catch {
            // Fallback: use the robust LLM parser (handles partial JSON too)
            chunkQuestions = validateQuestions(parseQuestionsFromLLM(rawText));
          }

          allQuestions.push(...chunkQuestions);

        } catch (err: any) {
          addLog(`  Chunk ${ci + 1} error: ${err.message}`, 'warn');
          // Don't break — try remaining chunks; partial results are still useful
        }
      }
      ...
      if (allQuestions.length === 0) {
        addLog(`Groq ${model.split('/').pop()} returned no valid questions`, 'warn');
        continue;
      }

      // Re-number IDs sequentially across all chunks
      allQuestions.forEach((q, i) => { q.id = i + 1; });
      addLog(`✓ Groq ${model.split('/').pop()} extracted ${allQuestions.length} questions`, 'ok');
      return allQuestions;
    }
  ...
  const tryGeminiText = async (text: string, key: string, label: string): Promise<Question[] | null> => {
    ...
    // Use hardcoded priority list directly — skip model listing API call to save time
    ...
    for (const model of models) {
      ...
      for (let ci = 0; ci < chunks.length; ci++) {
        ...
        const prompt =
          `Extract ALL MCQ questions from this text (chunk ${ci + 1}/${chunks.length}).\n` +
          `Return ONLY a valid JSON array — no markdown, no explanation:\n` +
          `[{"id":1,"question":"...","options":{"A":"...","B":"...","C":"...","D":"..."},"answer":"B"}]\n` +
          `If no MCQs in this chunk return []\n\nTEXT:\n${chunk}`;
        try {
...
Make sure every question has all options and the correct answer letter.`;
  ...
  // ── Deduplicate merged questions from two providers
  // Compares first 60 chars of lowercased question text — catches duplicates near split boundary.
  const deduplicateQuestions = (questions: Question[]): Question[] => {
    const seen = new Set<string>();
    const result: Question[] = [];
    for (const q of questions) {
      const key = q.question.toLowerCase().replace(/\s+/g, ' ').trim().slice(0, 60);
  ...
  // ── Main extraction: v7 — 65/35 split-work parallel extraction
  // When both keys present: Groq handles first 65% of pages, Gemini handles last 35%.
  // Both run simultaneously → total time ≈ max(groq_time, gemini_time) instead of sum.
  // Results are merged, deduplicated, and re-numbered.
  // Falls back to single-provider if only one key is set, or if PDF is scanned (Gemini vision).
  const startExtraction = async () => {
    ...
    try {
      ...
      // ── SPLIT-WORK PARALLEL (both keys present)
      if (bothProviders) {
        addLog('Extracting PDF pages...', 'info');
        const pages = await extractPdfPages(file);
        const totalPages = pages.length;
        const pdfOk = pages.join('').length >= 100;

        if (pdfOk) {
          // 65% pages → Groq  |  35% pages → Gemini (by page count, clean boundary)
          const splitIdx = Math.max(1, Math.round(totalPages * 0.65));
          const groqPages  = pages.slice(0, splitIdx);
          const geminiPages = pages.slice(splitIdx);

          const groqText   = groqPages.join('\n').trim();
          const geminiText = geminiPages.join('\n').trim();

          addLog(
            `⚡ Split: Groq → pages 1–${splitIdx} (${Math.round(groqText.length/1000)}k chars) | ` +
            `Gemini → pages ${splitIdx+1}–${totalPages} (${Math.round(geminiText.length/1000)}k chars)`,
            'info'
          );
          addLog('Running both providers in parallel...', 'info');

          // Fire both simultaneously — no racing, both complete their slice
          const [groqResult, geminiResult] = await Promise.allSettled([
            tryGroq(groqText, grKey),
            tryGeminiText(geminiText, gKey, 'Gemini'),
          ]);

          const groqQs   = groqResult.status   === 'fulfilled' ? (groqResult.value   ?? []) : [];
          const geminiQs = geminiResult.status === 'fulfilled' ? (geminiResult.value ?? []) : [];

          if (groqResult.status === 'rejected')
            addLog(`Groq slice failed: ${(groqResult.reason as any)?.message}`, 'warn');
          if (geminiResult.status === 'rejected')
            addLog(`Gemini slice failed: ${(geminiResult.reason as any)?.message}`, 'warn');

          const merged = deduplicateQuestions([...groqQs, ...geminiQs]);

          if (merged.length > 0) {
            addLog(
              `✓ Merged: ${groqQs.length} (Groq) + ${geminiQs.length} (Gemini) → ${merged.length} unique questions`,
              'ok'
            );
          ...
          // Both slices failed — fall through to Gemini vision as last resort
          addLog('Both text slices returned no questions — trying Gemini vision fallback...', 'warn');
          const visionResult = await tryGemini(file, gKey);
          if (visionResult && visionResult.length > 0) {
            await finishExtraction(visionResult, 'gemini-vision', file.name);
            return;
          }
          throw new Error('All providers returned no questions. PDF may have no extractable text.');

        } else {
          // Scanned PDF — text too short, use Gemini vision only
          addLog('PDF appears scanned — using Gemini vision mode...', 'warn');
          const valid = await tryGemini(file, gKey);
          if (valid && valid.length > 0) {
            await finishExtraction(valid, 'gemini-vision', file.name);
            return;
          }
      ...
      // ── SINGLE PROVIDER fallback paths
      if (grKey) {
        ...
        if (pdfText && pdfText.length >= 100) {
          ...
          if (valid && valid.length > 0) {
            await finishExtraction(valid, 'groq:llama-3.3-70b', file.name);
            return;
          }
          throw new Error('Groq could not extract questions. Try adding a Gemini key for vision mode.');
        } else {
      ...
      if (gKey) {
        ...
        const valid = await tryGemini(file, gKey);
        if (valid && valid.length > 0) {
          await finishExtraction(valid, 'gemini', file.name);
          return;
        }
