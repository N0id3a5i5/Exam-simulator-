...
export default function App() {
  ...
  // The caller pre-splits its text into page-slices (each ~13% of the PDF).
  // Each slice is dispatched to a Groq model in parallel (round-robin across 3 models).
  // All slices fire simultaneously — a rate-limit on one model only fails that slice,
  // not the whole extraction. Completed slices are merged by the caller.
  //
  // groqSlices: array of text strings, one per page-slice
  // gKey: Groq API key
  // Returns: flat array of questions from all successful slices (IDs not yet renumbered)
  const GROQ_MODELS = [
  ...
  const groqCallOne = async (
    ...
    if (data.error?.code === 429 || data.error?.message?.toLowerCase().includes('rate')) {
      addLog(`[Groq:${shortName}] slice ${sliceIdx + 1} rate-limited — skipped`, 'warn');
...
Make sure every question has all options and the correct answer letter.`;
  ...
  // ── Main extraction: v10 — 5-slice worker-pool + Gemini full-PDF safety net
  //
  // When both keys present:
  //   • PDF pages split into up to 5 Groq slices (~13% each) + 1 Gemini slice (35%)
  //   • All 5 Groq slices + Gemini 35% fire simultaneously in parallel
  //   • Groq slices round-robin across 3 models — rate-limit on one model only loses that slice
  //   • Gemini also receives the FULL PDF text as a fallback (runs concurrently)
  //   • If all Groq slices rate-limited → Gemini full-PDF result used instantly (no extra wait)
  ...
  const startExtraction = async () => {
    ...
    try {
      ...
      // ── PARALLEL WORKER-POOL (both keys present)
      if (bothProviders) {
        ...
        if (pdfOk) {
          // ── Build Groq slices: 5 slices of ~13% each covering first 65% of pages
          const groqEndIdx = Math.max(1, Math.round(totalPages * 0.65));
          const groqPages  = pages.slice(0, groqEndIdx);
          const NUM_SLICES = Math.min(5, groqPages.length); // fewer slices if PDF is short
          const sliceSize  = Math.ceil(groqPages.length / NUM_SLICES);
          const groqSlices: string[] = [];
          for (let s = 0; s < NUM_SLICES; s++) {
            const slicePages = groqPages.slice(s * sliceSize, (s + 1) * sliceSize);
            const sliceText  = slicePages.join('\n').trim();
            if (sliceText.length > 20) groqSlices.push(sliceText);
          }

          // ── Gemini slice: last 35% of pages
          const geminiSliceText = pages.slice(groqEndIdx).join('\n').trim();
          const geminiHasSlice  = geminiSliceText.length >= 50;

          addLog(
            `⚡ Groq: ${groqSlices.length} slices across pages 1–${groqEndIdx} | ` +
            `Gemini: pages ${groqEndIdx + 1}–${totalPages} + full-PDF safety net`,
            'info'
          );

          // ── Fire everything simultaneously
          // groqSlices → worker-pool (round-robin models, returns [] per failed slice)
          // geminiSlice → 35% tail pages
          // geminiFullPromise → full PDF, safety net if Groq is all rate-limited
          const groqPromise        = tryGroq(groqSlices, grKey);
          const geminiSlicePromise = geminiHasSlice
            ? tryGeminiText(geminiSliceText, gKey, 'Gemini-35%')
            : Promise.resolve([] as Question[]);
          const geminiFullPromise  = tryGeminiText(fullText, gKey, 'Gemini-full');

          const [groqResult, geminiSliceResult, geminiFullResult] = await Promise.allSettled([
            groqPromise,
            geminiSlicePromise,
            geminiFullPromise,
          ]);
          ...
          const geminiSliceQs = geminiSliceResult.status === 'fulfilled' ? (geminiSliceResult.value ?? []) : [];
          const geminiFullQs  = geminiFullResult.status  === 'fulfilled' ? (geminiFullResult.value  ?? []) : [];

          const groqGotSomething = groqQs.length > 0;

          if (groqGotSomething) {
            // ── Path A: Groq got questions → merge with Gemini 35% slice
            const merged = deduplicateQuestions([...groqQs, ...geminiSliceQs]);
            addLog(
              `✓ Groq: ${groqQs.length} + Gemini-35%: ${geminiSliceQs.length} → ${merged.length} unique questions`,
              'ok'
            );
          ...
          if (geminiFullQs.length > 0) {
            // ── Path B: Groq fully rate-limited → use Gemini full-PDF result (already done)
            addLog(`[Groq] All slices rate-limited — using Gemini full-PDF result (${geminiFullQs.length} questions)`, 'warn');
            await finishExtraction(geminiFullQs, 'gemini', file.name);
            return;
          }

          // ── Path C: Everything failed → try Gemini vision (scanned PDF)
          addLog('Text extraction failed — trying Gemini vision (scanned PDF?)...', 'warn');
          ...
          addLog('PDF appears scanned (no text) — using Gemini vision...', 'warn');
      ...
      if (grKey) {
        ...
        if (fullText.length >= 100) {
          // Build 5 slices covering full PDF for single-provider mode
          const NUM_SLICES = Math.min(5, pages.length);
          const sliceSize  = Math.ceil(pages.length / NUM_SLICES);
          const slices: string[] = [];
          for (let s = 0; s < NUM_SLICES; s++) {
            const t = pages.slice(s * sliceSize, (s + 1) * sliceSize).join('\n').trim();
            if (t.length > 20) slices.push(t);
          }
          addLog(`PDF extracted — ${slices.length} slices → Groq...`, 'info');
