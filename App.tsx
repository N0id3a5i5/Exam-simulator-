...
try {
  ...
  const allModels: string[] = mData.models
    .filter((m: any) => m.supportedGenerationMethods?.includes('generateContent'))
    .map((m: any) => m.name as string);

  // Priority order: try newest/fastest first, fall back through all available
  const priorityOrder = [
    'gemini-2.0-flash',
    'gemini-2.5-flash',
    'gemini-1.5-flash',
  ...
  const sortedModels = [
    ...priorityOrder
      .map(p => allModels.find(n => n.includes(p)))
      .filter(Boolean) as string[],
    ...allModels.filter(n => !priorityOrder.some(p => n.includes(p))),
  ];

  if (sortedModels.length === 0) throw new Error('No compatible Gemini model found');
  addLog(`Found ${sortedModels.length} model(s) — will auto-switch if quota exceeded`);
...
```
  ...
  for (const model of sortedModels) {
    ...
    addLog(`Trying model: ${modelShortName}...`);
    ...
    if (isQuotaError(data)) {
      addLog(`⚠ ${modelShortName} quota exceeded — trying next model...`, true);
      lastError = modelShortName;
      continue;
    }
  ...
  throw new Error(
    `All ${sortedModels.length} models have hit their free-tier quota limit.\n\n` +
    `To fix this:\n` +
    `1. Wait until midnight Pacific Time for daily quota to reset\n` +
    `2. Add billing at console.cloud.google.com (paid tier = much higher limits)\n` +
    `3. Create a new project + API key at aistudio.google.com/app/apikey`
  );

} catch (err: any) {
  const msg = err.message || String(err);
  addLog(`CRITICAL ERROR: ${msg}`, true);
  alert(`Error: ${msg}`);
  setLoading(false);
}
```
...
```
  for (const model of modelsToTry) {
    ...
    const data = await res.json();
    if (isQuotaError(data)) continue; // try next model
    if (data.error) throw new Error(data.error.message);
  ...
  throw new Error('All models quota exceeded. Try again later or add billing.');
} catch (err: any) {
...
// ── Upload Screen ────────────────────────────────────────────────────────────
const hasError = logs.some(l => l.includes(‘CRITICAL ERROR’));
...
{hasError && !loading && <p style={{ color: ‘var(–red)’, fontWeight: 700, marginBottom: 12 }}>❌ Processing failed — error details below:</p>}
<div className="log-console">
{logs.map((log, i) => <div key={i} dangerouslySetInnerHTML={{ __html: log }} />)}
<div ref={logsEndRef} />
</div>
{hasError && !loading && (
<button className=“btn-primary” style={{ marginTop: 16 }} onClick={() => { setLogs([’> System initialized…’]); }}>
← Try Again
</button>
)}
</div>
