...
// ── v13: NVIDIA NIM integration — adds 2 extra parallel workers (llama-4-maverick +
//         nemotron-super-49b) via OpenAI-compatible integrate.api.nvidia.com endpoint.
//         NVIDIA vision model (llama-3.2-11b-vision) replaces Gemini as primary OCR
//         engine for scanned pages, preserving Gemini quota. Key stored as nvidia_api_key.
//         UI shows "NVIDIA Active" badge and "nvapi-..." input field alongside Groq/Gemini.
...
// Fix #4  — Two-column layout (VisionIAS, UPSC papers): detect column boundary and reconstruct
//           left column first, then right column, so Q1-Q50 left + Q51-Q100 right reads correctly.
const extractPdfPages = async (
  ...
  for (let i = 1; i <= pdf.numPages; i++) {
    ...
    // ── Two-column detection: if items span both halves of the page, split at midpoint
    const xs = items.map((it: any) => it.transform[4]);
    const minX = Math.min(...xs), maxX = Math.max(...xs);
    const midX = pageWidth / 2;
    const leftItems  = items.filter((it: any) => it.transform[4] < midX);
    const rightItems = items.filter((it: any) => it.transform[4] >= midX);
    const isTwoColumn = leftItems.length > 5 && rightItems.length > 5
      && (maxX - minX) > pageWidth * 0.4; // columns span >40% of page width

    const reconstruct = (itemSet: any[]): string => {
      // Sort top-to-bottom, then left-to-right within same row (~3px tolerance)
      const sorted = [...itemSet].sort((a: any, b: any) => {
        const dy = Math.round(b.transform[5] / 3) - Math.round(a.transform[5] / 3);
        if (dy !== 0) return dy;
    ...
    let text: string;
    if (isTwoColumn) {
      // Reconstruct left column first, then right — preserves Q1..Q50 | Q51..Q100 order
      const leftText  = reconstruct(leftItems);
      const rightText = reconstruct(rightItems);
      text = leftText + (leftText && rightText ? '\n' : '') + rightText;
    } else {
      text = reconstruct(items);
    }
...
// ── v13: OCR scanned pages via NVIDIA vision (llama-3.2-11b-vision-instruct).
// Preferred over Gemini OCR when nvidia key is available — preserves Gemini quota.
// Uses OpenAI-compatible chat/completions with image_url content type.
const ocrScannedPagesWithNvidia = async (
  scannedPageImages: Record<number, string>,
  nvKey: string,
  addLog: (msg: string, kind: 'ok' | 'err' | 'warn' | 'info') => void
): Promise<Record<number, string>> => {
  ...
  const pageIndices = Object.keys(scannedPageImages).map(Number);
  addLog(`[OCR] Running NVIDIA vision OCR on ${pageIndices.length} scanned page(s)...`, 'info');
  const recovered: Record<number, string> = {};
  ...
  for (const pageIdx of pageIndices) {
    ...
    if (!base64) continue;
    try {
      const res = await capFetch('https://integrate.api.nvidia.com/v1/chat/completions', {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          messages: [{
            role: 'user',
            content: [
              { type: 'text', text: 'Extract ALL text from this exam page exactly as it appears. Preserve question numbers, options (A/B/C/D or 1/2/3/4), and Hindi/English text. Return plain text only.' },
      ...
      if (!text) { addLog(`[OCR] NVIDIA Page ${pageIdx + 1} empty — skipped`, 'warn'); continue; }
      recovered[pageIdx] = text;
      addLog(`[OCR] NVIDIA Page ${pageIdx + 1} recovered (${text.length} chars)`, 'ok');
    } catch (e: any) {
      addLog(`[OCR] NVIDIA Page ${pageIdx + 1} error: ${e.message} — skipped`, 'warn');
    }
  }
  return recovered;
};
...
export default function App() {
  ...
  const [showNvidiaInput, setShowNvidiaInput] = useState(false);
  ...
  useEffect(() => {
    (async () => {
      await initDB();
      const [tests, attempts, gKey, grKey, nvKey] = await Promise.all([
        dbGetSavedTests(),
        dbGetAttempts(),
        dbGetSetting('gemini_api_key'),
        dbGetSetting('groq_api_key'),
        dbGetSetting('nvidia_api_key'),
      ]);
      setSavedTests(tests);
      setAttemptHistory(attempts);
      if (gKey) setApiKey(prev => prev || gKey);
  ...
  // ── NVIDIA NIM extraction workers (v13)
  // OpenAI-compatible endpoint — same request shape as Groq, different base URL + key.
  // llama-4-maverick: large context, strong instruction following for complex UPSC formats.
  // nemotron-super-49b: NVIDIA reasoning-tuned model, excellent structured JSON output.
  const NVIDIA_MODELS = [
    { model: 'meta/llama-4-maverick-17b-128e-instruct', maxTokens: 16000 },
    { model: 'nvidia/llama-3.3-nemotron-super-49b-v1',  maxTokens: 16000 },
  ];

  const nvidiaCallOne = async (
    sliceText: string, sliceIdx: number, totalSlices: number,
    model: string, maxTokens: number, nvKey: string
  ): Promise<Question[]> => {
    ...
    const prompt =
      ...
      `- If options are labelled 1/2/3/4 or a/b/c/d, normalize to A/B/C/D. Use roman numerals only if ALL options use that scheme.\n` +
      `- For "Which of the following statements is/are correct?" questions, numbered sub-statements (1. 2. 3.) are PART of the question text, NOT option labels.\n` +
      `- The "answer" field MUST be one of: A, B, C, or D (the letter only).\n` +
      `- If a question references a diagram/figure/graph/map/circuit, include it and prefix with "[Visual question]".\n` +
      `- For Assertion-Reason: preserve option text exactly e.g. "Both A and R are true and R is the correct explanation of A".\n` +
      `- For Match-the-Column: encode List-I vs List-II in the question text. A/B/C/D = matching combos.\n` +
    ...
    addLog(`[NVIDIA:${shortName}] slice ${sliceIdx + 1}/${totalSlices} → sending...`, 'info');
    const data = await capFetch('https://integrate.api.nvidia.com/v1/chat/completions', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${nvKey}` },
      body: JSON.stringify({
        model,
    ...
    if (data.error?.code === 429 || data.error?.message?.toLowerCase().includes('rate') || data.error?.message?.toLowerCase().includes('too large')) {
      addLog(`[NVIDIA:${shortName}] slice ${sliceIdx + 1} rate-limited — skipped`, 'warn');
      throw new Error('rate-limited');
    }
    if (data.error) {
      addLog(`[NVIDIA:${shortName}] slice ${sliceIdx + 1} error: ${data.error.message}`, 'warn');
      throw new Error(data.error.message);
    }
    if (data.choices?.[0]?.finish_reason === 'length')
      addLog(`[NVIDIA:${shortName}] slice ${sliceIdx + 1} truncated — partial recovery`, 'warn');
    ...
    addLog(`[NVIDIA:${shortName}] slice ${sliceIdx + 1} ✓ ${qs.length} questions`, 'ok');
    return qs;
  };
  const groqCallOne = async (
    ...
    const prompt =
      `Extract ALL multiple choice questions from this PDF text (slice ${sliceIdx + 1}/${totalSlices}).\n` +
      `CRITICAL — THIS IS A UPSC/COMPETITIVE EXAM PAPER. COMMON PATTERNS:\n` +
      `- The paper likely has TWO COLUMNS per page. Left column has questions (e.g. Q1-Q5), right column has more questions (e.g. Q6-Q10). Read ALL questions from BOTH columns — do NOT miss the right column.\n` +
      `- The answer key appears at the END of the document as "Q 1. C", "Q 2. B" etc. Match each question number to its answer letter.\n` +
      `- If this slice contains an answer key section (lines like "Q 1. C  Q 2. B  Q 3. A"), extract answers from it and pair with questions found elsewhere in the document.\n` +
      `EXTRACTION RULES:\n` +
      `- Include ALL question types: standard MCQ, True/False, Assertion-Reason (Statement-I/Statement-II), Match-the-Column, Statement-based.\n` +
      `- If options are labelled 1/2/3/4 or a/b/c/d, normalize to A/B/C/D. Use roman numerals only if ALL options follow that scheme.\n` +
      `- For "Which of the following statements is/are correct?" questions, numbered sub-statements (1. 2. 3.) are PART of the question text, NOT option labels.\n` +
      `- The "answer" field MUST be one of: A, B, C, or D (the letter only).\n` +
      `- If a question references a diagram/figure/graph/map, include it and prefix with "[Visual question]".\n` +
      `- For Assertion-Reason: preserve option text exactly e.g. "Both Statement-I and Statement-II are correct and Statement-II is the correct explanation".\n` +
      `- For Match-the-Column: encode List-I vs List-II in the question text. A/B/C/D = matching combos.\n` +
      `- Return ONLY a valid JSON array. No markdown, no explanation:\n` +
      `[{"id":1,"question":"...","options":{"A":"...","B":"...","C":"...","D":"..."},"answer":"B"}]\n` +
      `- If no MCQs in this slice, return []\n` +
    ...
    for (const model of models) {
      ...
      for (let ci = 0; ci < chunks.length; ci++) {
        ...
        const prompt =
          ...
          `CRITICAL — THIS IS A UPSC/COMPETITIVE EXAM PAPER. COMMON PATTERNS:\n` +
          `- The paper likely has TWO COLUMNS per page. Left column has questions (e.g. Q1, Q3, Q5), right column has more (e.g. Q2, Q4, Q6). Read ALL questions from BOTH columns — do NOT miss the right column.\n` +
          `- The answer key appears at the END as "Q 1. C", "Q 2. B" etc. Match each question number to its answer letter.\n` +
          `- If this chunk contains an answer key section (lines like "Q 1. C  Q 2. B"), extract answers and pair with questions found in this or other chunks.\n` +
          `EXTRACTION RULES:\n` +
          `- Include ALL question types: standard MCQ, True/False, Assertion-Reason (Statement-I/Statement-II), Match-the-Column, Statement-based.\n` +
          `- If options are labelled 1/2/3/4 or a/b/c/d, normalize to A/B/C/D. Use roman numerals only if ALL options follow that scheme.\n` +
          `- For "Which of the following statements is/are correct?" questions, numbered sub-statements (1. 2. 3.) are PART of the question text, NOT option labels.\n` +
  ...
  const startExtraction = async () => {
    ...
    const nvKey = nvidiaKey.trim();

    if (!gKey && !grKey && !nvKey) { alert('Please enter at least one API key (Gemini, Groq, or NVIDIA)!'); return; }

    if (gKey) await dbSetSetting('gemini_api_key', gKey);
    if (grKey) await dbSetSetting('groq_api_key', grKey);
    if (nvKey) await dbSetSetting('nvidia_api_key', nvKey);

    setLoading(true);
    setLogs([{ msg: 'System initialized', kind: 'info' }]);

    try {
      const bothProviders = !!(gKey && grKey) || !!(grKey && nvKey) || !!(gKey && nvKey) || !!(grKey && gKey);

      // ── FULL-COVERAGE PARALLEL WORKER-POOL (any 2+ keys present)
      if (bothProviders) {
        addLog('Extracting PDF pages...', 'info');
        const { pages, scannedPageCount, scannedPageImages } = await extractPdfPages(file);
        const totalPages = pages.length;

        // OCR scanned pages — prefer NVIDIA vision, fall back to Gemini
        ...
        if (scannedPageCount > 0 && Object.keys(scannedPageImages).length > 0) {
          ...
          addLog(`⚠ ${scannedPageCount}/${totalPages} pages image-only (${pct}%). Running OCR...`, 'warn');
          const ocrResults = nvKey
            ? await ocrScannedPagesWithNvidia(scannedPageImages, nvKey, addLog)
            : await ocrScannedPagesWithGemini(scannedPageImages, gKey, addLog);
          ...
          addLog(
            ...
            `${pct > 50 ? 'Switching to Gemini vision...' : 'Text workers will skip blank pages.'}`,
            'warn'
          );
        }
        ...
        if (pdfOk) {
          ...
          // v13: 7 workers when NVIDIA key present (Groq×3 + Gemini×2 + NVIDIA×2), else 5
          const NUM_WORKERS = nvKey ? 7 : 5;
          const NUM_SLICES = Math.min(totalPages, Math.max(NUM_WORKERS, 2 * spf));
          ...
          addLog(
            `⚡ v13: ${totalPages} pages → ${allSlices.length} slices across ${NUM_WORKERS} workers ` +
            `[Groq×3${gKey ? ' + Gemini×2' : ''}${nvKey ? ' + NVIDIA×2' : ''}]`,
            'info'
          );
          ...
          const groqWorkerPromise = (workerIdx: number): Promise<Question[]> => {
            ...
            if (slices.length === 0 || !grKey) return Promise.resolve([]);
            return Promise.all(
              slices.map((sliceText, localIdx) => {
                const globalIdx = workerIdx + localIdx * NUM_WORKERS;
          ...
          // Workers 5-6: NVIDIA
          const nvidiaWorkerPromise = (workerIdx: number): Promise<Question[]> => {
            const { model, maxTokens } = NVIDIA_MODELS[workerIdx - 5];
            const slices = workerSlices[workerIdx];
            if (slices.length === 0 || !nvKey) return Promise.resolve([]);
            return Promise.all(
              slices.map((sliceText, localIdx) => {
                const globalIdx = workerIdx + localIdx * NUM_WORKERS;
                return nvidiaCallOne(sliceText, globalIdx, allSlices.length, model, maxTokens, nvKey)
                  .catch(() => [] as Question[]);
              })
            ).then(r => r.flat());
          };

          const [w0, w1, w2, w3, w4, w5, w6] = await Promise.allSettled([
            groqWorkerPromise(0),
            groqWorkerPromise(1),
            groqWorkerPromise(2),
            geminiWorkerPromise(3, 'Gemini-A'),
            geminiWorkerPromise(4, 'Gemini-B'),
            nvKey ? nvidiaWorkerPromise(5) : Promise.resolve([]),
            nvKey ? nvidiaWorkerPromise(6) : Promise.resolve([]),
          ]);
          ...
          const groqTotal = gather(w0).length + gather(w1).length + gather(w2).length;
          const geminiTotal = gather(w3).length + gather(w4).length;
          const nvidiaTotal = gather(w5).length + gather(w6).length;
          const rawTotal = groqTotal + geminiTotal + nvidiaTotal;
          addLog(
            `✓ Workers done — Groq: ${groqTotal} | Gemini: ${geminiTotal}${nvKey ? ` | NVIDIA: ${nvidiaTotal}` : ''} | raw: ${rawTotal} → deduped: ${merged.length}`,
    ...
    <div className="upload-wrap">
      {(loading || hasError) ? (
        ...
        <div className="upload-card">
          ...
          {/* ── NVIDIA key (optional — 2 extra workers + vision OCR) */}
          {!nvidiaKey ? (
            <>
              {showNvidiaInput ? (
                <div style={{ marginBottom: 14 }}>
                  <div className="field-label" style={{ color: '#76b900', textShadow: '0 0 8px rgba(118,185,0,0.3)' }}>
                    <Zap size={11} />NVIDIA API Key <span style={{ fontSize: 9, marginLeft: 4, fontFamily: 'Rajdhani', textTransform: 'none', letterSpacing: 0 }}>(optional — 2 extra workers + vision OCR)</span>
                  </div>
                  <div className="key-wrap">
                    <input className="key-input" style={{ borderColor: 'rgba(118,185,0,0.25)' }}
                      type={showNvidiaKey ? 'text' : 'password'} value={nvidiaKey}
                      onChange={e => { setNvidiaKey(e.target.value); dbSetSetting('nvidia_api_key', e.target.value); }}
                      placeholder="nvapi-…  (free at build.nvidia.com)" autoComplete="off" spellCheck={false} />
                    <button className="key-toggle" onClick={() => setShowNvidiaKey(s => !s)} tabIndex={-1}>
                      {showNvidiaKey ? <EyeOff size={15} /> : <Eye size={15} />}
                    </button>
                  </div>
                  <p className="field-hint" style={{ color: '#76b900' }}><Zap size={11} />Free at build.nvidia.com — adds Maverick + Nemotron workers</p>
                </div>
              ) : (
                <div className="groq-row" onClick={() => setShowNvidiaInput(true)} style={{ marginBottom: 14, borderColor: 'rgba(118,185,0,0.2)', background: 'rgba(118,185,0,0.04)' }}>
                  <div>
                    <div className="groq-row-label" style={{ color: '#76b900' }}><Zap size={13} />Add NVIDIA Key (Optional)</div>
                    <div className="groq-row-sub">+2 extra workers · Vision OCR · Free tier</div>
                  </div>
                  <span className="groq-badge" style={{ color: '#76b900', background: 'rgba(118,185,0,0.1)', borderColor: 'rgba(118,185,0,0.2)' }}>⚡ +2</span>
                </div>
              )}
            </>
          ) : (
            <div style={{ marginBottom: 14, display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(118,185,0,0.06)', border: '1px solid rgba(118,185,0,0.25)', borderRadius: 10, padding: '9px 13px' }}>
              <span style={{ display: 'flex', alignItems: 'center', gap: 7, fontSize: 12.5, color: '#76b900', fontWeight: 600 }}>
                <Zap size={13} />⚡ NVIDIA Active (+2 workers)
              </span>
              <button onClick={() => { setNvidiaKey(''); dbSetSetting('nvidia_api_key', ''); setShowNvidiaInput(false); }}
                style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--muted)', fontSize: 11, fontFamily: 'Rajdhani', fontWeight: 600, padding: '2px 6px', borderRadius: 6 }}
                onMouseEnter={e => (e.currentTarget.style.color = 'var(--red)')}
                onMouseLeave={e => (e.currentTarget.style.color = 'var(--muted)')}>Remove</button>
            </div>
          ...
          <button className="btn-primary" onClick={startExtraction} disabled={!file || (!apiKey.trim() && !groqKey.trim() && !nvidiaKey.trim())}>
            {nvidiaKey.trim() ? '⚡ Start Extraction (Multi-AI) →' : groqKey.trim() ? '⚡ Start Extraction (Groq) →' : 'Start Extraction →'}
          </button>
        </div>
      )}
