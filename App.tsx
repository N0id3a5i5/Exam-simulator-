...
// Detects scanned pages (image-only) and flags them.
// Returns per-page text array so callers can split at page boundaries.
const extractPdfPages = async (file: File): Promise<{ pages: string[]; scannedPageCount: number }> => {
  ...
  for (let i = 1; i <= pdf.numPages; i++) {
    ...
    const text = content.items.map((item: any) => (item as any).str).join(' ').trim();
    pages.push(text);
    if (text.length < 30) scannedPageCount++; // likely image-only page
  }
  return { pages, scannedPageCount };
};

// ── Stitch page text so questions are never silently split at slice boundaries.
// Strategy: always include a 300-char overlap from the PREVIOUS page at the start
// of the next page, so a question that starts at the bottom of page N is visible
// to the slice that processes page N+1 as well.
const buildSlicesWithOverlap = (pages: string[], numSlices: number, overlapChars = 300): string[] => {
  ...
  for (let s = 0; s < numSlices; s++) {
    ...
    // Prepend tail of previous slice as context overlap
...
// Handles: 1/2/3/4, a/b/c/d, i/ii/iii/iv, (1)/(2), (a)/(b), etc.
const normalizeOptions = (q: any): { options: Record<string, string>; answer: string } => {
  ...
  // Build a mapping: original key → A/B/C/D
  ...
  keys.forEach((k, idx) => {
    ...
    const mapped = LABEL_MAP[stripped] || String.fromCharCode(65 + idx); // fallback A,B,C…
    normOptions[mapped] = raw[k];
    keyRemap[stripped] = mapped;
    keyRemap[k.trim()] = mapped; // also map original casing
...
// These have descriptive combo answers like "Both A and R are true..."
// We keep them as-is but tag them so the UI can render a note.
const isAssertionReason = (q: any): boolean => {
  const opts = Object.values(q.options || {}) as string[];
  return opts.some(v =>
    typeof v === 'string' && /both.*true|assertion.*reason|neither.*nor/i.test(v)
  );
};
...
export default function App() {
  ...
  const groqCallOne = async (
    ...
    const prompt =
      ...
      `IMPORTANT RULES:\n` +
      `- Include ALL question types: standard MCQ, True/False, Assertion-Reason, Match-the-Column.\n` +
      `- If options are labelled 1/2/3/4 or a/b/c/d or i/ii/iii/iv, normalize them to A/B/C/D.\n` +
      `- The "answer" field MUST be one of: A, B, C, or D (the letter only, not the text).\n` +
      `- If a question references a diagram/figure, still include it — add "[Visual question]" at the start.\n` +
      `- For Match-the-Column: encode the column as part of the question text, and the answer choices as A/B/C/D combos.\n` +
  ...
  const tryGeminiText = async (text: string, key: string, label: string): Promise<Question[] | null> => {
    ...
    for (const model of models) {
      ...
      for (let ci = 0; ci < chunks.length; ci++) {
        ...
        const prompt =
          `You are an MCQ extractor. Extract ALL multiple-choice questions from the text below.\n` +
          `IMPORTANT RULES:\n` +
          `- Include ALL question types: standard MCQ, True/False, Assertion-Reason, Match-the-Column.\n` +
          `- If options are labelled 1/2/3/4 or a/b/c/d or i/ii/iii/iv, normalize them to A/B/C/D.\n` +
          `- The "answer" field MUST be one of: A, B, C, or D (the letter only).\n` +
          `- If a question references a diagram/figure, still include it — add "[Visual question]" at the start.\n` +
          `- For Match-the-Column: encode the column mapping as part of the question text.\n` +
  ...
  const tryGemini = async (file: File, key: string): Promise<Question[] | null> => {
    ...
    const prompt =
      `You are an MCQ extractor. Read this PDF and extract ALL multiple-choice questions.\n` +
      `IMPORTANT RULES:\n` +
      `- Include ALL question types: standard MCQ, True/False, Assertion-Reason, Match-the-Column.\n` +
      `- If options are labelled 1/2/3/4 or a/b/c/d or i/ii/iii/iv, normalize them to A/B/C/D.\n` +
      `- The "answer" field MUST be one of: A, B, C, or D (the letter only).\n` +
      `- For questions with diagrams/figures you cannot read: include the question text and prefix it with "[Visual question]". Use your best judgment for options.\n` +
      `- For Match-the-Column: encode column A items vs column B items in the question text, and the A/B/C/D options should be the matching combinations.\n` +
  ...
  const startExtraction = async () => {
    ...
    try {
      ...
      if (bothProviders) {
        ...
        const { pages, scannedPageCount } = await extractPdfPages(file);
        const totalPages = pages.length;
        const fullText = pages.join('\n').trim();
        const pdfOk = fullText.length >= 100;

        // Warn user if PDF has many image-only pages
        ...
        if (pdfOk) {
          ...
          const allSlices = buildSlicesWithOverlap(pages, NUM_SLICES);
          ...
          addLog(`PDF appears scanned (${scannedPageCount}/${pages.length} blank pages) — using Gemini vision...`, 'warn');
      ...
      if (grKey) {
        ...
        const fullText = pages.join('\n').trim();
        if (scannedPageCount > 0)
          addLog(`⚠ ${scannedPageCount}/${pages.length} pages appear image-only — add a Gemini key for vision fallback.`, 'warn');
        if (fullText.length >= 100) {
          // Build 5 slices with overlap covering full PDF
          const NUM_SLICES = Math.min(5, pages.length);
          const slices = buildSlicesWithOverlap(pages, NUM_SLICES);
