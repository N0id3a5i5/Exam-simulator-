...
const httpPost = async (url: string, headers: Record<string, string>, body: object): Promise<any> => {
  ...
  if (cap?.isNativePlatform?.()) {
    ...
    if (Http) {
      const res = await Http.request({ method: 'POST', url, headers, data: body });
...
// ── FIX: Enhanced JSON repair that handles truncated/incomplete JSON from LLMs
const repairTruncatedJson = (raw: string): string => {
  ...
  for (; i < raw.length; i++) {
    ...
    if (inStr) continue;
    if (ch === '[') arrDepth++;
    else if (ch === ']') { arrDepth--; if (arrDepth === 0) lastCompleteObjEnd = i; }
    else if (ch === '{') objDepth++;
    else if (ch === '}') {
      objDepth--;
      // Track last fully-closed object at array level 1
      if (objDepth === 0 && arrDepth === 1) lastCompleteObjEnd = i;
    }
  }

  // If JSON is incomplete (arrDepth > 0), truncate to last complete object and close array
  if (arrDepth > 0 && lastCompleteObjEnd > 0) {
...
const parseQuestionsFromLLM = (raw: string): Question[] => {
  ...
  // If no closing bracket, the JSON was truncated — use full string from '['
  ...
  // Step 4: repair truncated JSON (handles mid-output cutoffs)
  ...
  try {
    ...
    // Step 6: recovery — fix unquoted keys and single quotes
    try {
      const fixed = cleaned
      ...
      if (gKey) {
        ...
        try {
          ...
          if (pdfText && pdfText.length >= 100) {
            // ── FIX: Limit input to avoid truncated output.
            // Groq models have a large context window but limited OUTPUT tokens.
            // Sending too much input forces a long response that gets cut off mid-JSON.
            // 18000 chars of input leaves enough headroom for complete JSON output.
            const MAX_INPUT_CHARS = 18000;
            const truncatedText = pdfText.slice(0, MAX_INPUT_CHARS);
            const wasTextTruncated = pdfText.length > MAX_INPUT_CHARS;
            if (wasTextTruncated) {
              addLog(`PDF text truncated to ${MAX_INPUT_CHARS} chars to prevent output cutoff`, 'warn');
            }
              ...
              try {
                const gData = await httpPost(
                  ...
                  { 'Content-Type': 'application/json', 'Authorization': `Bearer ${gKey}` },
                  {
                    ...
                    messages: [
                      {
                        // ── FIX: System prompt instructs model to always close JSON fully
                        role: 'system',
                        content: 'You are a precise JSON extractor. Output ONLY a complete, valid JSON array. NEVER truncate. If all questions do not fit, output fewer complete questions — never output partial/incomplete JSON objects. Always close all brackets and braces.'
                      },
                      {
                        role: 'user',
                        content: `Extract ALL MCQ questions from this text. Return ONLY a raw JSON array with NO markdown, NO explanation:\n[{"id":1,"question":"full question text","options":{"A":"...","B":"...","C":"...","D":"..."},"answer":"B"}]\n\nIMPORTANT: The JSON array MUST be complete and syntactically valid. Never cut off mid-object.\n\nPDF TEXT:\n${truncatedText}`
                      }
                    ],
                    temperature: 0.1,
                    // ── FIX: max_tokens raised from 8000 to 32768
                    // The previous 8000 limit caused JSON to be cut off mid-object
                    // for large PDFs. Groq supports up to 32768 output tokens.
                    max_tokens: 32768
                  }
                );
                ...
                if (gData.error) { addLog(`Groq ${gModel} error: ${gData.error.message} — trying next model`, 'warn'); continue; }

                const rawContent = gData.choices?.[0]?.message?.content as string;
                if (!rawContent) { addLog(`Groq ${gModel} returned empty response — trying next`, 'warn'); continue; }

                // ── FIX: Check finish_reason to detect truncated output
                const finishReason = gData.choices?.[0]?.finish_reason;
                if (finishReason === 'length') {
                  addLog(`Groq ${gModel} output was truncated (finish_reason=length) — attempting JSON repair`, 'warn');
                }

                const parsed = parseQuestionsFromLLM(rawContent);
                const valid = validateQuestions(parsed);
                if (valid.length === 0) { addLog('Groq returned no valid questions — trying next model', 'warn'); continue; }

                if (finishReason === 'length') {
                  addLog(`✓ Recovered ${valid.length} questions from truncated Groq output`, 'ok');
                }
                finishExtraction(valid, `groq:${gModel}`);
                return;
              } catch (modelErr: any) {
                addLog(`Groq ${gModel}: ${modelErr.message} — trying next`, 'warn');
                continue;
              }
            }
            // All Groq models failed — fall through to Gemini
            addLog('All Groq models failed — falling back to Gemini', 'warn');
          } else {
              ...
              {showGroqInput ? (
                <div style={{ marginBottom: 14 }}>
                  <div className="field-label" style={{ color: 'var(--yellow)', textShadow: '0 0 8px rgba(255,230,0,0.3)' }}>
                    <Zap size={11} />Groq API Key <span style={{ fontSize: 9, marginLeft: 4, fontFamily: 'Rajdhani', textTransform: 'none', letterSpacing: 0 }}>(optional — used first when present)</span>
                  </div>
                  <div className="key-wrap">
                    <input
