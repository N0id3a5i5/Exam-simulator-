  ...
  useEffect(() => {
    ...
    if (screen === 'test' && clock > 0) {
      interval = setInterval(() => {
        setClock(c => {
          if (c <= 1) { clearInterval(interval); setScreen('results'); return 0; }
          return c - 1;
        });
      }, 1000);
    }
...
Make sure every question has all options and the correct answer letter.`;
  ...
  const fetchExplanation = async (q: Question) => {
    ...
    try {
      ...
      const prompt = `Explain clearly why "${q.answer}" is the correct answer for this MCQ question. Keep it to 2-3 sentences only. Be direct and educational. Question: ${q.question} Options: ${Object.entries(q.options).map(([k, v]) => `${k}. ${v}`).join(', ')} Correct Answer: ${q.answer}. ${q.options[q.answer]}`;
  ...
  const saveResults = () => {
    if (resultsRef.current) {
      ...
      if (printWindow) {
        printWindow.document.write(`<html><head><title>Exam Report</title><style>
          body { font-family: Arial, sans-serif; padding: 20px; }
          ${GLOBAL_CSS}
        </style></head><body>${resultsRef.current.innerHTML}</body></html>`);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', padding: 16 }}>
      {loading ? (
        ...
        <div className="upload-card">
          ...
          <button className="btn-primary" onClick={startExtraction} style={{ marginTop: 16 }}>
            Start Extraction →
          </button>
        </div>
      )}
